<template>
    <div>
        ...
        <div v-if="active === 0" class="step-content">
            ...
            <el-input type="textarea" v-model="dubText" placeholder="输入文本" :rows="18" clearable style="width: 90%;"
...
const saveInput = () => {
    localStorage.setItem('dubText', dubText.value);
    savedInput.value = dubText.value;
};

const clearInput = () => {
    dubText.value = '';
    savedInput.value = '';
    localStorage.removeItem('dubText');
};

const handleFileUpload = (file) => {
    const reader = new FileReader();
    reader.onload = (e) => {
        dubText.value = e.target?.result || ''; // 确保 dubText 是一个字符串
        validateInputLength();
    };
    if (
...
onMounted(() => {
    const storedInput = localStorage.getItem('dubText');
    if (storedInput) {
        savedInput.value = storedInput;
        dubText.value = storedInput;
    }
});

// 格式化 savedInput，每 160 个字符换行
const formattedSavedInput = computed(() => {
    return savedInput.value.match(/.{1,150}/g)?.join('\n') || '';
});

const isInputTooShort = computed(() => dubText.value.length > 0 && dubText.value.length < 800);
const isInputTooLong = computed(() => dubText.value.length > 2000);
